...
def numWaterBottles(self, numBottles, numExchange):
        ...
        :rtype: int
        """
        # Approach 1 complexity O(1)
        #return numBottles + ((numBottles -1 ) / (numExchange - 1))

        # Approach 2 complexity O(log numBottles)
        result = numBottles
        empty_bottles = numBottles
        while(empty_bottles >= numExchange):
            full_bottles = empty_bottles // numExchange
            remaning_bottles = empty_bottles % numExchange
            result = result + full_bottles
            empty_bottles = full_bottles + remaning_bottles
        return result
